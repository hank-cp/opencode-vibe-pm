...
- \`dictionary.md\` → 本地语言 ↔ 英文术语转换
  ...
  buildInitInstructions(packs: LanguagePack[]): string {
    ...
    return JSON.stringify({
      ...
      steps: [
        { id: "language", title: "交互语言", type: "question", instruction: "询问用户选择交互语言。用户选择后，用所选语言调用 pm_config init language=<locale> 获取后续步骤。", params: { header: "Language / 语言", question: "选择 vibe-pm 交互语言 / Choose vib-pm interactive language:", options: languageOptions }, onAnswer: languageOnAnswer, nextAction: "调用 pm_config init subCommand=init language=<locale>" },
      ],
    });
  },
