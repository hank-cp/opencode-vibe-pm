...
| PluginConfig | `language`, `dataDir`, `autoAnalyze`, `contextInjection` | 从 `vibe-pm/config.json` 加载 |
...
sequenceDiagram
    ...
    OC->>PC: Plugin 初始化
    PC->>Config: ensureDefaultConfig() 创建 vibe-pm/config.json（如不存在）
    Config-->>PC: 配置就绪
...
启动时确保 `vibe-pm/config.json` 存在（不存在则创建默认配置）。目录结构（`docs/flow/` 等）由 `/pm-install-flow` 按需创建。
...
config: async (opencodeConfig) => {
  ...
  // pm-config init 提供交互式初始化（不再使用独立的 pm-init 命令）
...
| `/pm-config` | config（声明式）| 查看或修改 vibe-pm/config.json 配置 |
...
function loadConfig(projectDir: string): PluginConfig {
  const configPath = path.join(projectDir, "vibe-pm", "config.json");

  if (!fs.existsSync(configPath)) {
    // 返回默认配置，由 /pm-config init 创建
...
- **Setup/Teardown**: 创建临时目录和 vibe-pm/config.json 文件，测试后清理

| 动作指令 | 测试方法 | Given | When | Then | Notes |
|----------|----------|-------|------|------|-------|
| 新增 | `loadConfig_defaults` | `vibe-pm/config.json` 不存在 | 调用 `loadConfig()` | 返回 DEFAULT_CONFIG | 首次使用的项目 |
| 新增 | `loadConfig_override` | `vibe-pm/config.json` 存在，`language: "en-US"` | 调用 `loadConfig()` | 返回合并后的配置（language=en-US，其余默认） | 部分覆盖 |
| 新增 | `loadConfig_invalid_json` | `vibe-pm/config.json` 内容为非法 JSON | 调用 `loadConfig()` | 返回 DEFAULT_CONFIG + 记录 warning | 容错处理 |
...
| 新增 | `init_creates_config_if_missing` | `vibe-pm/config.json` 不存在 | Plugin 初始化 | 自动创建默认配置文件 | 首次使用的项目 |
...
| 场景 | 预期行为 |
|------|---------|
| `vibe-pm/config.json` 不存在 | 自动创建默认配置文件 |
...
- 不联网（当前阶段）
- 不删除任何用户文件
- 配置文件变更需 `/pm-config init` 或 `/pm-refine-flow` 触发，不能自动修改
...
### [x] 里程碑 1 — Plugin Core MVP

- [x] Plugin Core 入口（`VibePMPlugin`）+ 启动时自动创建默认配置
- [x] 配置管理（`loadConfig` + `DEFAULT_CONFIG` + `ensureDefaultConfig`，支持深度合并/自动创建）
- [x] 8 个 `/pm-*` 命令注册（config hook + tool hook）
- [x] SDK 日志系统（通过 OpenCode `app.log()` 写入宿主日志）
  - 已知问题/风险: `plugin.ts:60` 处 `// TODO debug code, need continuous observation` — flowSessions 收集但仅打日志未实际使用
- [x] FlowEngine 集成（任务自动创建 + `<pm-control-rules>` 注入 + 控制提示清理）
  - 已知问题/风险: `experimental.chat.messages.transform` 在后续 OpenCode 版本中可能被移除或合并
- [x] 核心类型定义（`PluginConfig`, `IPluginContext`, `ILogger`，OpenCode 交互类型来自 SDK）
- [x] DCP 集成配置保护（`src/integration/dcp.ts` — protectTags + protectedFilePatterns）
