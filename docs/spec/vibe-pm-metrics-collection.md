...
| **Tools**       | part.type === "tool" 或 part.role === "tool" | 工具调用/返回 |
...
| messages.transform 中 parts 为空数组 | TokenCountResult.total = 0，不写入数据库 |
...
| SQLite (better-sqlite3) | 不修改版本，通过预编译语句 INSERT/SELECT/UPDATE 操作 |
