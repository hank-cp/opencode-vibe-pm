...
**版本**: 1.2.0 | **批准日期**: 2026-06-11 | **最后修订**: 2026-06-26
...
### VI. 代码品质优先（Code Quality First）

每个代码变更都应力求优雅，直达问题核心。代码必须是可阅读和可维护的。

- 代码的阅读者比编写者更重要 — 写一次，读百次
- 修改时顺手清理坏味道 — 发现周围代码有改进空间就改进，不因条框而搁置
- 抽象要有明确收益 — 为可读性、可测试性、可复用性而抽象，不为抽象而抽象
- 命名即文档 — 通过清晰的命名表达意图
...
### IX. 尊重人工修改（Respect Manual Changes）

在任务进行过程中，若用户手工修改了代码并明确告知，AI 必须：

1. **读取并理解**修改内容，分析修改目的和设计意图
2. **禁止**在未获得用户明确同意的情况下，自行修改、覆盖或回退用户重构过的代码
3. 若用户的修改引入了可验证问题（测试失败、类型错误等），应在获得用户同意后再进行调整

此原则是对代码品质优先原则（VI）的补充：代码品质不仅是结构性的，也包含对他人设计意图的尊重。
