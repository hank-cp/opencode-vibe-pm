...
| FSM / State Machine   | 状态机     | 用 Mermaid 定义步骤流转逻辑                                |
| Human-in-loop         | 人机协作    | 需要用户介入的步骤                                         |
| Step Transition       | 步骤流转    | 从一个步骤推进到下一个步骤                                     |
| Context Injection     | 上下文注入   | 根据当前步骤向对话注入相关内容                                   |
| Flow Control Prompt   | 流程控制提示词 | 用于控制流程执行的提示词                                      |
