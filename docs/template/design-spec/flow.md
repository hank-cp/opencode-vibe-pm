...
3. 输出 Spec 文档到 `/docs/spec/{feature-name}.md`
4. 不适用于当前设计的章节标注「不适用」后保留骨架
5. 标注设计中仍待决策的开放项（如有）
6. **评估需求规模**：判断需求是否过大、是否可以分解为多个层级/模块分步实施：
   - 判断标准：涉及 3 个以上独立模块 或 预估实现周期超过 1 个 sprint 或 子功能间有明确先后依赖
   - 若需求规模较大且可分解 → 生成**分解方案**
   - 若需求粒度适中、不宜再拆分 → 标注「无需分解」，跳过以下 7-8 项
7. **生成分解方案**（若适用），包含：
   - 分解后的子模块/子项目列表，每个子模块需明确职责边界和交付物
   - 各子模块间的依赖关系（使用 Mermaid 流程图或表格标注）
   - 建议的分步实施顺序（Phase 1 → Phase 2 → ...），每个 Phase 定义独立交付价值
   - 每个 Phase 的 MVP 定义——实现该 Phase 后用户能独立验证什么
8. 分解方案作为 Spec 文档的独立章节（如「## 分步实施计划」）输出，或根据规模输出为独立文档
9. 若无需分解，在 Spec 中标注「本需求粒度适中，作为整体实施」
...
> **⚠️ 本步骤需要用户介入。** 用户审查 Spec 文档及分解方案（如有），确认后完成。

**目标**：用户审查 Spec 文档，确认设计满足需求；若 S5 输出了分解方案，一并确认是否按分解方案分步实施。

1. 展示 Spec 文档摘要——核心设计决策、关键用户故事、主要约束和风险
2. 若 S5 生成了分解方案，一并展示分解方案摘要：
   - 子模块列表及职责边界
   - Phase 依赖关系和实施顺序
   - 各 Phase 的独立交付价值
3. 若存在分解方案，使用 `confirm` 工具询问用户：「是否按分解方案分步实施？」
   - 用户确认分解 → 记录分解决策，后续各 Phase 可通过独立的设计/开发任务推进
   - 用户选择不分解 → 保持整体 Spec，按单任务推进
4. 使用 `confirm` 工具等待用户对 Spec 整体审查确认
5. 审查通过后，使用 `question` 工具询问用户：「是否执行 `git commit`？」
   - 若用户选择「是」：执行 `git add -A && git commit`，使用本次 Spec 设计的总结作为 commit message
   - 若用户选择「否」：跳过提交
   - ⚠️ 用户选择不影响任务结束

> ⚠️ **重要**：Spec 设计到 S6 为止。本流程的交付物是 Spec 文档（含可选的分解方案），**不进入代码实现**。后续如需实现，应通过 `/pm-spec-driven-dev` 或 `/pm-new-feature` 启动独立任务。
